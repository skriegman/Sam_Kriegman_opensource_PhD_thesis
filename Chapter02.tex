\chapter{Structure}

\textbf{Appeared as:}\\
S.~Kriegman et al.,
\href{https://ieeexplore.ieee.org/abstract/document/9116004}{\color{blue}Scalable sim-to-real transfer of soft robot designs}.
In
\textit{Proceedings of the IEEE International Conference on Soft Robotics (RoboSoft)} (2020).


\vspace{1em}

\noindent
\textbf{Abstract:}\\
\input{Chapter02/0_abstract}


\begin{figure}[t]
    \centering
    \includegraphics[width=\linewidth]{Chapter02/fig/roboSoftTeaser.png}
    \caption{The top 100 simulated 2-by-2-by-2 configurations of passive (cyan) and volumetrically-actuating (red) voxels (\textbf{a}) were manufactured in reality (\textbf{b}).
    } 
    \label{fig:teaser}
\end{figure}


% \subsubsection*{Video 2.1: Manufacturing process.}
% Link: \href{https://youtu.be/jbQ2T7jIYRU}{\textbf{\texttt{youtu.be/jbQ2T7jIYRU}}}


% \subsubsection*{Video 2.2: Sim and real behavior.}
% Link: \href{https://youtu.be/UqjvmkYa9u4}{\textbf{\texttt{youtu.be/UqjvmkYa9u4}}}



\input{Chapter02/1_table1.tex}

\input{Chapter02/1_intro}



\input{Chapter02/2_methods}

\input{Chapter02/2_fig_pressure.tex}

\input{Chapter02/2a_methods_design_space.tex}

\input{Chapter02/2b_methods_sim.tex}

% \input{Chapter02/2_fig_real.tex}

\input{Chapter02/2c_methods_real.tex}


\input{Chapter02/3_results}
% \input{Chapter02/3_fig_sim.tex}
% \input{Chapter02/3_fig_transfer.tex} 
% \input{Chapter02/3_fig_hi_res_sim.tex}


\input{Chapter02/4_discussion}

% \input{Chapter02/2_table2.tex}








