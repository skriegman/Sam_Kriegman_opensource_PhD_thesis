...
\begin{enumerate}
    ...
    \item Protean body plans introduce smaller and thus safer mutations \cite{lehman2018safe}, whose behavioral impact manifests temporarily during operation, rather than permanently from deployment (chapter 3).
    ...
    % Figs.~\ref{fig:baldwin2d} and \ref{fig:baldwin3d}.
    ...
    This well-studied phenomenon \cite{hinton1987learning} is revisited with a twist:
...
\begin{enumerate}
    ...
    % : it's just (squishy and sometimes moist) Lego bricks that can attach on all sides (not just dorsoventrally).
    % 
    ...
    the robot finds itself in a new field (Fig.~\ref{fig:ashbyStepMech}), capable of a previously unreachable behavior: brachiation.
...
When these parts change even slightly, such as by the lengthening of a single leg, the control policy could be rendered more or less obsolete.
...
% Either a small software patch or larger update will be necessary to regain control over the new hardware
In the worst case it will be easier to evolve a totally new controller or robot than to repair the existing system \cite{cheney2016difficulty}.

% then even slight changes to the body can result in catastrophic failure.
...
% at the bleeding edge of robotics 
proofs of concept---%
basic science
...
% material/structure/shape flexibility 
morphological
...
the system would also need to take control over the kinds of 
% actions 
configurations
...
and metamorphosis (for animals) is metabolically expensive.
