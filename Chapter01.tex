...
Organisms, like Proteus \cite{pesic2010shapes},
% of Greek mythology, 
...
\begin{enumerate}
    ...
    (chapter 5).
    The amount of generalization was found to depend on the kinds of interoceptive signals used to guide their morphological change during optimization.
...
\begin{enumerate}
    ...
    The 
    % relative  %%  sam: if shape changes the configuration then this cannot be relative but must be absolute
...
In 1994,
under the aegis of the Thinking Machines Corporation and the MIT Media Lab,
the very first evolved yet virtual robots were reported by Sims \cite{sims1994evolving,sims1994competition} and Ventrella \cite{ventrella1994explorations}: structure and control of autonomous agents were co-optimized so that they ran, jumped, swam, performed phototaxis, and fought head-to-head for resources in a virtual world that followed (more or less) the laws of classical mechanics. 
...
% Vaughn's work shows that passive dynamics can be a scaffold, not a disruptive phenomenon.
% Because now the controller does need to control this interaction, that no longer happens for free

The behavioral consequences of physical mutations or damage are particularly harsh when morphology is ontogenetically-static, because any change to behavior manifests instantly and permanently from deployment or time of damage.
...
However, as Hinton and Nowlan \cite{hinton1987learning} so neatly demonstrated: development can, under certain conditions, smooth the search space that evolution operates in, thereby creating a gradient toward increasingly better mutations (Fig.~\ref{fig:baldwin2d}).
...
\begin{figure}[t]
\centering
\includegraphics[trim={40pt 20pt 30pt 0pt},clip,width=0.49\linewidth]{fig/baldwin_rugged.pdf}
\includegraphics[trim={40pt 20pt 30pt 0pt},clip,width=0.49\linewidth]{fig/baldwin_smooth.pdf}
...
Evolution discretely samples the space by setting the initial conditions 
% ($x_0,y_0$)
(starting point)
and how development may unfold across the landscape (curvature and length of the line) in time (acceleration along the curve) until death. 
% ($x_{\tau},y_{\tau}$).
...
In the limit, the line of development shrinks 
% until $(x_0,y_0) = (x_{\tau},y_{\tau}) = (x^*,y^*)$.
to a point: the initial conditions.
...
\cite{dellaert1994toward,dellaert1996developmental,eggenberger1997evolving,Bongard01,miller2004evolving,basanta2008evolution,doursat2009organically,doursat2014growing,joachimczak2016artificial,mordvintsev2020growing}.
...
basic science
that will undoubtedly be refined with additional research and development.
...
But they do not have the requisite variety to recover from deeper mechanical damage, such as the removal of all limbs or being cut into a hundred smaller pieces.
...
% body parts
% in the first place.
The resiliency of such robots is limited not only because they cannot re-generate structure, motors or sensors---%
but because they cannot generate these
body parts
in the first place.
...
continually greater distance from the bias, intuition and cognitive limits of their human designer.
% \footnote{In the case of evolutionary robotics the designer crafts the algorithm that automatically designs robots.}
...
\cite{shepherd2011multigait,shah2019morphing,shah2020gaining,shah2020shape};
...
% this is agency autonomy, which is different
