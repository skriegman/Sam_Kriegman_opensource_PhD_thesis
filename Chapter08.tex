...
As machines become more and more protean, however, the number and kind of modalities that can vary and interact between timescales increases in exponential fashion.
As a first step,
we introduced
...
This suggests that, if structure is allowed to vary, automated design could be used as a filter to discover body plans which provide a small gaps before attempting to cross them with controller optimization.
...
The peak of morphological plasticity generally lined up with the start of an increasing trend in fitness and marked the onset of differential canalization:
The discovery of rolling at the very end of ontogeny, followed by a form of progenesis in which heterochronic mutations reduced morphological plasticity in each voxel, gradually preponing rolling from a late onset behavior to one that arose increasing earlier in ontogeny.
That is, shape was canalized; however, configuration patterns were not.
This was because an asymmetrical ball is a ``permissive body plan''.
Simply put, practically all oscillation patterns in an asymmetrical ball will result in locomotion (rolling).
Legged body shapes were not permissive: they were finely tuned to match their configuration oscillations (and vice versa). 
Moreover, they were brittle: a sequence of four or five random mutations would always break a legged walker.
Thousands of random mutations to the ball, in contrast, did not significantly affect its performance.
Whereas the ball shape could be refined and canalized without disruption, mutations to legged bodies needed to be precisely coordinated with mutations in configuration patterns and this prevented the canalization of highly fit legged morphologies.
...
This work demonstrated that robustness can be achieved by evolving the structure of robots, their configuration patterns, and how
...
Specifically, we observed that if one modality (stiffness) responds to one particular internal signal (engineering stress) but not another (pressure), robots evolved structures that intrinsically buffered large deviations from their expected material properties.
In other words, morphological change in response to the appropriate signals during evolution can foster ``zero-shot'' generalization to fabrication errors.
Although developmental reactions in response to pressure did not afford the evolution of robustness here, they did increase evolutionary divergence: 
...
Specifically,
it is currently unknown how cardiomyocytes will synchronize their contractions when reorganized into novel structures in vitro / in vivo.
...
The results suggest that ontogenetic morphological change can 
fortify autonomous machines against entropy,
and
confer the evolution of robust behavior,
...
Ultimately,
this knowledge will be put to work developing useful technologies, whether they are composed of electronics, living systems, or admixtures of both.
