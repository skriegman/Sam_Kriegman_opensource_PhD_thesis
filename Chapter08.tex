...
Evolution and development were therefore confined to select subsets of actions among the fixed configuration alternatives defined by the static body plan.
...
Specifically,
the evolution of open-loop shape change demonstrated that the explicit suppression of development enforced by \citet{hinton1987learning} is in fact not necessary: ballistic morphological change is enough to confer evolvability.


Later shape was allowed to vary in addition to configuration subsets.
...
% Traits that are robust to changes in other traits can become canalized while the others remain free to vary.
...
A temporally-clamped protean machine might even look like a conventional robot for an extended period of time, but unlike paradigmatic robots, when the environmental conditions change, a protean machine is not bounded by a static body plan with fixed sensorimotor constraints.
...
only changes the way the robot maps sensor data to actions: the sequences of configurations it will execute.
In addition to finding an appropriate mapping from sensors (or central-pattern-generators) to actions, there is also the problem of determining the basic categories of actions: configuration alternatives.
...
the same sequence of actions led to different configurations, different ways the the robot interacted with its environment to propel itself forward or influence objects (such as pellets or other agents; chapter 7) in its environment.
...
which will inevitably break down and fall apart---% 
but they cannot grow new body plans that regenerate or expand the set of configurations they have available.
It follows that a more protean machine can be more robust and resilient than an otherwise equivalent but less protean machine.
...
In each instance, structure and configuration-subsets were evolved, but the robot could not change its body plan (i.e.~configuration alternatives) during operation.
